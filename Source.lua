...
local LOAD_ENV = LPH_NO_VIRTUALIZE(function()
	if not (readfile or writefile or isfile) then
		local VirtualStorage = {}
		...
		getgenv().readfile = function(path)
			...
			return VirtualStorage[p] or ""
		end

		getgenv().isfile = function(path)
			local p = normalizePath(path)
			return VirtualStorage[p] ~= nil
		end

		getgenv().isfolder = function(path)
			local p = normalizePath(path)
			for k in pairs(VirtualStorage) do
		...
		getgenv().makefolder = function(path)
			-- virtual folder, no-op
		end

		getgenv().delfile = function(path)
			local p = normalizePath(path)
			VirtualStorage[p] = nil
		end
	end
end)
...
local CoreGui = (gethui and gethui()) or (get_hidden_gui and get_hidden_gui()) or (pcall(function() return cloneref(game:GetService('CoreGui')) end) and cloneref(game:GetService('CoreGui'))) or cloneref(LocalPlayer.PlayerGui);
...
GlobalWindow.AutoLocalize = false;
...
function NeverLose:CreateWindow(Config)
	...
	ConfigName.Size = UDim2.new(1, -70, 0, 15)
	ConfigName.ClipsDescendants = true
	ConfigName.ZIndex = 9
	ConfigName.Font = Enum.Font.GothamMedium
	ConfigName.Text = "Default"
	ConfigName.AutoLocalize = false
	...
	function Window:_InitConfig()
		...
		local UpdateSize = LPH_NO_VIRTUALIZE(function()
			...
			NeverLose.PlayAnimate(ConfigFrame,SlowyTween , {
				Size = UDim2.fromOffset(math.max(115, size.X + 75), 30)
			});
		end);
		...
		local mv = NeverLose:CreateInput(LoadConfig , function()
			...
			if cfg_name and cfg_name:gsub("%s+", "") ~= "" then
				...
				if #cfg_name > 24 then
					Logging.new("triangle-exclamation", "Límite: 24 caracteres", 3.5)
					is_creating = false
					return
				end
				...
				if not cfg_name:find('/',1,true) and not cfg_name:find('\\',1,true) then
					...
					while ConfigExistsCaseInsensitive(final_name) do
						...
						if #final_name > 24 then
							final_name = final_name:sub(1, 24)
						end
						counter = counter + 1
					end
					cfg_name = final_name
